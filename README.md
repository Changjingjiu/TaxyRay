...
接入你自己的 AI 服务 拍摄小票或从相册多选图片 AI 提取商品 实付金额与优惠信息 并建议商品分类 应用自动完成逐项价税拆分 你可以查看依据 调整金额与税率 再保存结果
...
| 核心能力 | 做了什么 |
| --- | --- |
| **AI 小票识别** | 拍完即识别 每轮可继续拍照或结束核对 相册每轮最多 5 张 同一张长小票可分多轮录入 |
| **重复项确认** | 相同或相似商品由你选择保留一个或全部保留 入账前也会提醒疑似已有账单 |
...
3. 回到首页 点击 **识别小票** 查看服务地址与费用说明 选择 **拍照并识别** 在相机确认照片后开始识别 相册每轮可选 1 至 5 张 排好顺序后点击发送
4. 每轮识别完成后选择 **继续拍照** 或 **结束并核对** 同一张长小票可分多轮录入 整次最多 1000 个商品项
5. 发现相似商品时选择 **删除重复项** 保留指定的一项 或 **全部保留** 核对金额与税率 有整单优惠时确认分摊 最后确认入账

每轮识别都会访问你配置的服务 可能产生 API 费用 后一轮失败不会丢掉前面的结果 可以重拍或先核对已有内容 不同小票请分开录入
